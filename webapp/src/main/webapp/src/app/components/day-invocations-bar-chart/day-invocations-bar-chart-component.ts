...
export class DayInvocationsBarChartComponent implements OnInit {
  ...
  ngOnInit() {
    let width = parseInt(d3.select('#dayInvocationsBarChart').style('width')),
        height = 340,
    ...
    this.data.subscribe(invocationsData => {
      let vis = d3.select('#dayInvocationsBarChart')
        .append('svg')
          .attr('width', width)
      ...
      let max = d3.max(d3.map(invocationsData.hourlyCount).values());
      ...
      let hourlyData = d3.entries(invocationsData.hourlyCount).sort(function(a, b) { return d3.ascending(parseInt(a.key), parseInt(b.key)); });
