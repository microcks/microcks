...
/**
 * Helper class for validating Soap Messages against their WSDL. Code here is
 * mainly extracted and adapted from
 * SoapUI WsdlValidator class (see
 * https://github.com/SmartBear/soapui/blob/main/soapui/src/main/java/com/eviware/soapui/impl/wsdl/support/wsdl/WsdlValidator.java
 * for more details)
 *
 * @author laurent
 */
public class SoapMessageValidator {
  ...
  /**
   * Validate a soap message accordingly to its WSDL and linked XSD resources. The
   * validation is
   * done for a specified message part (maybe be the input, output or fault of an
   * operation).
   *
   * @param partName            The name of the part to validate ie. name of the
   *                            input, output or fault part (ex: sayHello)
   * @param partNamespace       The namespace of the part to validate (ex:
   *                            http://www.mma.fr/test/service)
   * @param message             The full soap message as a string
   * @param wsdlUrl             The URL where we can resolve service and operation
   *                            WSDL
   * @param validateMessageBody Should we validate also the body ? If false, only
   *                            Soap envelope is validated.
   * @return The list of validation failures. If empty, message is valid !
   * @throws org.apache.xmlbeans.XmlException if given message is not a valid Xml
   *                                          document
   */
  public static List<XmlError> validateSoapMessage(String partName, String partNamespace, String message,
      String wsdlUrl, boolean validateMessageBody)
      throws XmlException {
    //
    ...
    if (validateMessageBody) {
      ...
      // Build the QName string of the part name. Ex:
      // {http://www.github.com/lbroudoux/service}sayHello
  ...
  private static void validateMessageBody(WsdlContext ctx, List<XmlError> errors, SchemaType type, XmlObject msg)
      throws XmlException {
    // Need to create new body element of correct type from xml text since we want
    // to retain line-numbers.
    XmlOptions xmlOptions = new XmlOptions();
    xmlOptions.setLoadLineNumbers();
    xmlOptions.setLoadLineNumbers(XmlOptions.LOAD_LINE_NUMBERS_END_ELEMENT);
    ...
    // Problem: prefixes might get redefined/changed when saving which can cause
    // xsi:type refs to
    // reference wrong/non-existing namespace.. solution would probably be to
    // manually walk through
    // document and update xsi:type refs with new prefix. The
    // setUseDefaultNamespace() above helps
