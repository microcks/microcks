...
public class OpenAPISchemaValidator {
   ...
   /**
    * Check if a Json object is valid against the given OpenAPI schema
    * specification.
    *
    ...
    * @param schemaText The OpenAPI schema specification as a string
    * @param jsonText   The Json object as a string
    * @param namespace  Namespace definition to resolve relative dependencies in
    *                   Json schema
    * @return True if Json object is valid, false otherwise
    * @throws IOException if string representations cannot be parsed
    */
   ...
   /**
    * Validate a Json object representing by its text against a schema object
    * representing byt its
    * text too. Validation is a deep one: its pursue checking children nodes on a
    * failed parent. Validation
    * is respectful of OpenAPI schema spec semantics regarding additional or
    * unknown attributes: schema must
    * explicitely set <code>additionalProperties</code> to false if you want to
    * consider unknown attributes
   ...
   /**
    * Validate a Json object representing an OpenAPI message (response or request)
    * against a node representing
    * a full OpenAPI specification (and not just a schema node). Specify the
    * message by providing a valid JSON pointer
    * for <code>messagePathPointer</code> within specification and a
    * <code>contentType</code> to allow finding the correct
    * schema information. Validation is a deep one: its pursue checking children
    * nodes on a failed parent. Validation
    * is respectful of OpenAPI schema spec semantics regarding additional or
    * unknown attributes: schema must
    * explicitly set <code>additionalProperties</code> to false if you want to
    * consider unknown attributes
    * as validation errors. It returns a list of validation error messages.
    *
    * @param specificationNode  The OpenAPI full specification as a Jackson node
    * @param jsonNode           The Json object representing actual message as a
    *                           Jackson node
    * @param messagePathPointer A JSON Pointer for accessing expected message
    *                           definition within spec
    ...
    *                           definition within spec
    * @param contentType        The Content-Type of the message to valid
    * @param namespace          Namespace definition to resolve relative
    *                           dependencies in OpenAPI schema
    * @return The list of validation failures. If empty, json object is valid !
    */
   public static List<String> validateJsonMessage(JsonNode specificationNode, JsonNode jsonNode,
      ...
      if (messageNode == null || messageNode.isMissingNode()) {
         ...
         return Arrays
               .asList("messagePathPointer does not represent an existing JSON Pointer in OpenAPI specification");
      }
    ...
    * the fact that OpenAPI spec may be formatted in YAML. In that case, it handles
    * the
    * conversion.
    *
